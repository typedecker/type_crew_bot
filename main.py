    ...
    while not client.is_closed():
        ...
        if timezones != [] :
            ...
            for tz_entry in timezones :
                ...
                morn_time = datetime.datetime(morn_time.year, morn_time.month, morn_time.day, 12, 54, 000000, tzinfo = morn_time.tzinfo)
                if morn_time <= now_for_tz :
                    ...
                    if len([msg for msg in good_morn_msgs if pytz.timezone(tz).localize(msg.created_at) >= morn_time and msg.author == client.user]) == 0 :
                        print('MY DUDE WE CHECKED WE CAN DO THE JOB DEFINITELY', tz, [pytz.timezone(tz).localize(msg.created_at).strftime("%y-%m-%d-%H-%M") for msg in good_morn_msgs if msg.author == client.user])
